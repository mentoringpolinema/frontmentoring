...
@section('content')
                                                                ...
                                                                <ul class="nk-store-statistics">
                                                                    <li class="item">
                                                                        <div class="info">
                                                                            ...
                                                                            <div class="count"> {{$totalUserMentor}} </div>
                                                                        </div>
                                                                        <em class="icon bg-primary-dim ni ni-users"></em>
                                                                    </li>
                                                                </ul>
                                            ...
                                            <div class="card-inner">
                                                <div class="card-title-group">
                                                    ...
                                                    <div class="card-search search-wrap" data-search="search">
                                                        <div class="search-content">
                                                            <a href="#" class="search-back btn btn-icon toggle-search" data-target="search"><em class="icon ni ni-arrow-left"></em></a>
                                                            <input type="text" class="form-control border-transparent form-focus-none" placeholder="Cari Data Mentor">
                                                            <button class="search-submit btn btn-icon"><em class="icon ni ni-search"></em></button>
                                                        </div>
                                                    </div><!-- .card-search -->
                                                </div><!-- .card-title-group -->
                                                ...
                                                <table class="datatable-init nk-tb-list nk-tb-ulist" data-auto-responsive="false">
                                                    ...
                                                    <tbody>
                                                        @foreach ($data_user_mentor as $mentor)
                                                        <tr class="nk-tb-item">
                                                            <td class="nk-tb-col">
                                                                <div class="user-card">
                                                                    <div class="user-info">
                                                                        <span class="tb-lead">{{$mentor->user->name}}<span class="dot dot-success d-md-none ml-1"></span></span>
                                                                    </div>
                                                                </div>
                                                            </td>
                                                            {{-- <td class="nk-tb-col tb-col-mb">
                                                            ...
                                                            <td class="nk-tb-col tb-col-md">
                                                                <form action="/admin/user/{{$mentor->slug}}" method="post" style="display: inline">
                                                                    @csrf
                                                                    @method('DELETE')
                                                                    <button class="btn btn-round btn-sm btn-danger" type="submit" class="fa fa-trash">Delete</button>
                                                                </form>
                                                                {{-- <a href="/admin/delUserPanitia/{{$panitia->id_panitia}}" class="btn btn-round btn-sm btn-danger"><span>Delete</span></a> --}}
                                                                <a href="#" class="btn btn-round btn-sm btn-primary" data-toggle="modal" data-target="#modalEditMentor" id="editUserMentor" data-id="{{$mentor->id_mentor}}"><span>Edit</span> </a>
                                                            </td>
                                                        </tr><!-- .nk-tb-item  -->                                                                                                         
                                                        @endforeach
                                                    </tbody>
            ...
            <div class="modal-content">
                ...
                <div class="modal-body modal-body-lg">
                    <h5 class="title">Edit Mentor</h5>
                            <form action="/admin/user/editMentor" class="form-validate is-alter" method="POST">
                            @csrf
                            @method('PUT')
                            <div class="row gy-4">
                                <input type="hidden" class="form-control form-control" id="id_mentor" name="id_mentor_edit" value="" required>

                                <div class="col-md-6">
                                    <div class="form-group">
                                        <label class="form-label" for="mentor-name">Nama</label>
                                        <input type="text" class="form-control form-control" id="nama_mentor" name="nama_mentor_edit" value="" required>
                                    </div>
                                </div>
                                <div class="col-md-6">
                                    <div class="form-group">
                                        <label class="form-label" for="email-mentor">Email</label>
                                        <input type="email" class="form-control form-control" id="email_mentor" name="email_mentor_edit" value="{{$mentor->user->email}}" required>
                                        {{-- <input type="email" class="form-control form-control" id="email_panitia" name="email_panitia_edit" value="" required> --}}
                                    </div>
                                </div>
                                <div class="col-md-6">
                                    <div class="form-group">
                                        <label class="form-label" for="status-mentor">Status</label>
                                        <select class="form-control" id="status_mentor" name="status_mentor_edit" value="{{$mentor->status_mentor}}" required>
                                            <option>- Pilih Status -</option>
                                            <option value="aktif">Aktif</option>
                                            <option value="tidak aktif">Tidak Aktif</option>
                                            {{-- @foreach ($data_user_panitia as $panitia)
    ...
    @section('jsAdd')
        ...
        $('body').on('click', '#editUserMentor', function (event) {
                event.preventDefault();
                var token = $("input[name='_token']").val();
                var id = $(this).data('id');
                // console.log(id)
                $.ajax({
                    url: "<?php echo route('getUserMentorByID') ?>",
                    method: 'POST',
                    data: {id_mentor:id, _token:token},
                    // dd($data);
                    success: function(data) {
                        $("input[name='id_mentor_edit']").val(data.mentor.id_mentor);
                        $("input[name='nama_mentor_edit']").val(data.mentor.nama_mentor);
                        $("input[name='email_mentor_edit']").val(data.user.email);
                        $("select[name='status_mentor_edit']").val(data.mentor.status_mentor).change();
                        // $("input[name='password_panitia_edit']").val(data.user.password);
                    }
                    // console.log(data);
                });
